...
from models.base.tokenizers import NeuralCodec, Wav2Vec
from data import (PreprocessedDataset, SoundDataset, get_dataloader,
                   get_preprocessed_dataloader, SongGenDataset)

from models.base.cond_transformers import TokenConditionedTransformer
from models.SingSong import CoarseStage, SemanticStage, SingSong
# from .open_musiclm import (CoarseStage, FineStage, SemanticStage, ContinuationStage,SingSongStage,
#                            TokenConditionedTransformer, ContinuationStage)
...
class SingleStageTrainer(nn.Module):
    ...
    def __init__(
        ...
        stage: Literal['semantic', 'coarse', 'fine', 'continuation', 'singsong'],
        *,
        num_train_steps,
        batch_size,
        model_config,
        ...
        # self.run = wandb.init(project = "singsong_new_1")self.run
        self.stage = stage
        ...
        if stage == 'semantic':
            ...
            if self.use_preprocessed_data:
                ...
                self.ds_fields = ('raw_accom_for_semantic', 'raw_vocal_for_acoustic', 'raw_accom_for_acoustic')
                target_sample_hz = (wav2vec.target_sample_hz, neural_codec.sample_rate, neural_codec.sample_rate)
                normalize = (True, False, False)
                seq_len_multiple_of = wav2vec.seq_len_multiple_of

        elif stage == 'singsong':
            print("In the SingSong stage")
            assert self.use_preprocessed_data or (exists(wav2vec) and exists(audio_conditioner) and exists(neural_codec))
            self.train_wrapper = SingSong(
                coarse_transformer=transformer,
                neural_codec=neural_codec,
                wav2vec=wav2vec,
                clap=audio_conditioner,
            ...
            if self.use_preprocessed_data:
                ...
                # self.ds_fields = ('raw_vocal_for_acoustic', 'raw_vocal_for_semantic', 'raw_accom_for_clap', 'raw_accom_for_acoustic')
                # target_sample_hz = (audio_conditioner.sample_rate, wav2vec.target_sample_hz, 48000, neural_codec.sample_rate)
                # normalize = (False, True, False, False)
                self.ds_fields = ('raw_vocal_for_acoustic', 'raw_vocal_for_semantic', 'raw_accom_for_semantic')
                target_sample_hz = neural_codec.sample_rate, wav2vec.target_sample_hz, wav2vec.target_sample_hz
                normalize = (False, True, True)
                seq_len_multiple_of = wav2vec.seq_len_multiple_of

        # elif stage == 'continuation':
        #     print("In the continuation stage")
        #     self.transformer = nn.DataParallel(self.transformer, device_ids=[0, 1, 2])
        #     self.train_wrapper = ContinuationStage(
        #         coarse_transformer=transformer,
        #         neural_codec=neural_codec,
        #         wav2vec=wav2vec,
        #         clap=audio_conditioner,
        ...
        #     )
        #     if self.use_preprocessed_data:
        #         self.ds_fields = ('coarse_token_ids', 'semantic_token_ids', 'coarse_token_ids')
        #     else:
        #         self.ds_fields = ('prev_wave_for_acoustic', 'raw_wave_for_semantic', 'raw_wave_for_acoustic')
        #         target_sample_hz = (neural_codec.sample_rate, wav2vec.target_sample_hz, neural_codec.sample_rate)
        ...
        self.reduce_lr = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optim, 'min', patience = 2000, factor =0.5)
        if lr_warmup > 0:
        ...
        if self.use_preprocessed_data:
            ...
            self.dl = get_dataloader(self.ds, batch_size=batch_size, num_workers = 64, shuffle=True)
            self.valid_dl = get_dataloader(self.valid_ds, batch_size=batch_size, num_workers = 64, shuffle=True)
        ...
        if stage == 'semantic':
            ...
            hps.update(asdict(self.model_config.continuation_cfg))
            hps.update(asdict(self.training_config.continuation_trainer_cfg))
    ...
    def train_step(self):
        ...
        for _ in tqdm(range(self.grad_accum_every), desc = f"Steps:{steps}"):
            ...
            while non_empty_batch is False:
                ...
                self.accelerator.backward(loss.mean() / self.grad_accum_every)

                accum_log(logs, {'loss': loss.mean().item() / self.grad_accum_every})
        ...
        if not (steps % self.save_results_every):
            ...
            while non_empty_batch is False:
                ...
                self.reduce_lr.step(valid_loss)
                ...
                # self.run.log({'valid_loss': valid_loss, 'valid_accuracy': valid_accuracy})
                ...
                if self.is_main and self.save_reconstructed_wave and (self.stage == 'coarse' or self.stage == 'continuation' or self.stage=='fine' or self.stage =='singsong'):
